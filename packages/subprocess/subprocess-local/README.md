...
Mount the provider beside its consumers and start processes exactly as the subprocess service specifies; this package decides only how those processes run on the host. On Windows, non-terminal children and `taskkill` helpers start with their windows hidden so background operations do not take focus. This also hides GUI windows that honor the process startup visibility setting. The Windows Job runner is itself launched console-less (`windowsHide`) and passes `CREATE_NO_WINDOW` to Job and confined targets, so command execution under a console-less parent (the packaged desktop) opens no console window.
