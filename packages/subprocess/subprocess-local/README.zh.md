...
把提供方与它的消费方挂载在同一组合中，并完全按子进程服务的规定启动进程；本包只决定这些进程在宿主机上如何运行。在 Windows 上，非终端子进程与 `taskkill` 辅助进程会隐藏窗口，因此后台操作不会抢占焦点。遵循进程启动可见性设置的 GUI 窗口也会被隐藏。Windows Job runner 自身以无控制台方式（`windowsHide`）启动，并向 Job 与受限目标传递 `CREATE_NO_WINDOW`，因此在无控制台的父进程（打包版桌面）下执行命令不会打开控制台窗口。
