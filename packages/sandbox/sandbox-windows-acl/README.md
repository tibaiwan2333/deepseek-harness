...
- **Console windows are suppressed** — confined children are created with `CREATE_NO_WINDOW` (owned by `@deepseek-ai/dsh-win32-process`), so a console-less parent (the packaged desktop) allocates no console window per command. The restricted token keeps the keep-alive restricting SIDs (logon SID, Everyone) that console initialization needs; real-runner runs of both modes exit cleanly, and the `STATUS_DLL_INIT_FAILED` (`0xC0000142`) failures previously recorded for `CREATE_NO_WINDOW` do not reproduce with the current keep-alive lists.
