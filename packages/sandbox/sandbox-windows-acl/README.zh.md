...
- **控制台窗口被抑制。** 受限子进程以 `CREATE_NO_WINDOW` 创建（由 `@deepseek-ai/dsh-win32-process` 所有），因此无控制台的父进程（打包版桌面）不会为每条命令分配可见的控制台窗口。受限 token 保留了控制台初始化所需的保活 restricting SID（登录 SID、Everyone）；两种模式的真实 runner 运行均正常退出，先前记录的 `CREATE_NO_WINDOW` 导致 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）的失败在当前保活列表下不再复现。
